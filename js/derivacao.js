...
// Conta quantas tarefas existem em cada etapa. Usada pelo painel
// "Fleet Overview" (js/estados.js) para projetar números que vêm
// sempre de estado.tarefas — nunca um valor fixo no HTML. Assim como
// derivarTarefasVisiveis, é uma função pura: recebe um array, devolve
// um objeto novo, nunca lê nem escreve o DOM.
export function contarPorStatus(tarefas) {
  return tarefas.reduce(
    (contagem, tarefa) => {
      if (contagem[tarefa.status] !== undefined) {
        contagem[tarefa.status] += 1;
      }
      return contagem;
    },
    { "a-fazer": 0, "em-andamento": 0, "em-revisao": 0, concluida: 0 }
  );
}
