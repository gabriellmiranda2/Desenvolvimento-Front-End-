...
async function iniciar() {
  ...
  try {
    ...
    if (erro.name === "TypeError") {
      ...
      mensagem =
        "Não foi possível conectar ao arquivo de casos. Verifique sua conexão com a rede.";
    } else if (erro.name === "SyntaxError") {
      // resposta.json() lança SyntaxError quando o corpo não é um
      // JSON válido (ex.: vírgula sobrando, chave sem aspas).
      mensagem = "Os dados do arquivo não estão em um formato válido.";
      ...
      mensagem = `Não foi possível carregar os casos (erro ${erro.status}).`;
    } else {
      mensagem = "Ocorreu um erro inesperado ao carregar os casos.";
    }
